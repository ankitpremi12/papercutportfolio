...
export default function Certifications() {
  return (
    <section id="certifications" className="py-20 md:py-32 max-w-7xl mx-auto px-6 md:px-12 space-y-24">
      ...
      <div className="space-y-12">
        ...
        >
          {achievements.map((ach, i) => {
            const rotations = [1, -1.2, 0.8, -1.5];
            const rotation = rotations[i % rotations.length];
            const variants = ["blue", "dark", "cream", "navy"] as const;
            const variant = variants[i % variants.length];

            return (
              <motion.div key={ach.title} variants={cardVariant} className="h-full">
                <PaperCard
                  variant={variant}
                  rotation={rotation}
                  className="p-6 h-full flex flex-col justify-center"
